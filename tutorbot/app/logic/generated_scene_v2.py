...
class AnimationScene(Scene):
    def construct(self):
        ...
        # --- bytes to array to Manim ImageMobject
        ...
        ill_w, ill_h = img.pixel_array.shape[1], img.pixel_array.shape[0]
        ...
        while True:
            ...
            frame = cv2.resize(frame, (ill_w, ill_h))
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            frames.append(ImageMobject(frame))

        cap.release()


        self.play(Transform(img, frames[0]), 
                  run_time = 1)

        for frame_img in frames[1:]:
            self.add(frame_img)
            self.wait(1/30)  # 30 FPS
            self.remove(frame_img)

        self.wait(illustration_narration_duration - (len(frames) * (1 / 30)))
